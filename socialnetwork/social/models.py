...
class Post(models.Model):
    ...
    def create_tags(self):
        for word in self.body.split():
            if(word[0] == '#'):
                tag = Tag.objects.filter(name=word[1:]).first() # filter vas retourner une liste a laquelle on vas prendre le 1er element
                if tag:
    ...
    def create_tags(self):
        for word in self.comment.split():
            if(word[0] == '#'):
                ...
                if tag:
                    self.tags.add(tag.pk) ## si le tag existe on lui ajoute l'element
                else: # sinon on en créé un nouveau et on ajoute le nouvel élément a ce dernier
                    tag = Tag(name=word[1:])
                    tag.save()
                    self.tags.add(tag.pk)
                
                self.save() # on suvegarde
...
class Notification(models.Model):
    # 1 = Like,2 = commentaires,3 = Follow,4 = DM
