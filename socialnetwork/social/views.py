...
"""
Ici on crée une classe de post pour recuperer tous les posts qui ont été éffectué
"""
...
class PostDetailView(LoginRequiredMixin,View):
    def get(self,request,pk,*args,**kwargs):
        post = Post.objects.get(pk=pk)# on recupere les postes avec leur clé primaire
        form = CommentForm()
...
class ThreadNotification(View):
    def get(self,request,notification_pk,object_pk,*args,**kwargs):
        notification = Notification.objects.get(pk= notification_pk)
        profile = ThreadModel.objects.get(pk=object_pk)
...
class Explore(View):
    def get(self,request,*args,**kwargs):
        ...
        if tag:
            posts = Post.objects.filter(tags__in=[tag]) # ici on cherche si le tag est dans la liste des tags
        else:
            posts = Post.objects.all() # ici c'Est parceque l'on a mis un meta classe dans Post qui vas nous donner de prendre en fonction de tout ca
